...
export const listStoresByFilters = async (req: Request, res: Response): Promise<void> => {
    ...
    if (!name || typeof name !== 'string' || name.trim() === '') {
        res.status(400).json({ message: 'Por favor, proporciona un nombre válido para buscar.' });
        return;
    }
...
export const addStore = async (req: Request, res: Response): Promise<void> => {
    ...
    if (!name || !description || !address || !latitude || !longitude || !phone || !email) {
        res.status(400).json({ message: 'All fields are required' });
        return;
    }

    try {
        const newStore = new Store({ name, description, address, latitude, longitude, phone, email });
